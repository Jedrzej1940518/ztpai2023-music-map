...
const SignIn = ({
  ...
  return (
    <div
      className={`sign-in-container ${showSignedIn ? 'show' : ''}`}
      onMouseEnter={() => setShowSignOptions(true)}
      onMouseLeave={() => setShowSignOptions(false)}
    >
      {signedIn ? (
        <>
          <div className='header-text'>Logged in as {userData.nickname}</div>
          <div
            className={`sign-out-option ${showSignOptions ? 'show' : ''}`}
            onClick={() => {
